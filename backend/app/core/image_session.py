"""Warm/persistent Kaggle image sessions — W.0 CPU echo proof-of-loop.
...
W.0 proves the loop with a CPU echo kernel — no GPU, no model: start a session →
submit a job → the kernel echoes the prompt back into the job row → stop. The
real FLUX serve-loop and the full unified job layer arrive in W.1.
...
def _session_title() -> str:
    ...
    return KAGGLE_SESSION_SLUG.replace("-", " ").title()
...
def start_session(
    ...
    The push itself starts the Kaggle run, which loops on Supabase until the timer
    expires, the image cap is hit, or the session is stopped. Returns immediately.
    """
    get_image_model(model)  # validate model id (→ 400) before anything else
    ...
    source = kaggle.inject_payload(
        KAGGLE_SESSION_POC_TEMPLATE.read_text(encoding="utf-8"), payload
    )
    # CPU only (no GPU/dataset) for the W.0 echo proof; internet on so the kernel
    # can reach Supabase. Single non-blocking push — the run loops on its own.
    kaggle.deploy_kernel(
        username=cfg["username"],
        api_token=cfg["api_token"],
        kernel_name=KAGGLE_SESSION_SLUG,
