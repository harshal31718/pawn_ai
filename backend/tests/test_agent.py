...
async def test_load_context_node_no_longer_retrieves(mock_dispatch, mock_retrieve):
    """Phase M, M.4: load_context_node must NOT touch memory at graph start
    -- retrieved_memory starts empty; only search_memory_node populates it,
    on the agent's own decision."""
    state = {
        "conversation_id": "c1",
...
async def test_search_memory_node(mock_dispatch, mock_retrieve):
    mock_retrieve.return_value = [{"text": "found fact", "conv_id": "c1"}]
    state = {
        "conversation_id": "c1",
        "history": [],
        "retrieved_memory": [],
...
@patch("app.memory.retrieve.retrieve", new_callable=AsyncMock)
@patch("app.agent.graph.adispatch_custom_event", new_callable=AsyncMock)
async def test_search_memory_node_stateless_never_queries(mock_dispatch, mock_retrieve):
    """A chat with no resolved scope (stateless) must never query memory."""
    state = {
    ...
    res = await search_memory_node(state)
    mock_retrieve.assert_not_called()
    assert res["scratchpad"][0]["result"] == "No relevant memory found."
